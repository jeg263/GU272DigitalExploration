...
function buildSankey() {
    ...
    function resetData(data) {
        ...
        var louisianaExists = csvJSON(data, true).filter(function(p) { return (p.buyer_name === 'KLouisiana');});
        var newData = csvJSON(data, true).filter(function(p) { return (p.id !== null && p.id !== "" && p.id !== undefined);});
        ...
        var buyerFields;
        if (louisianaExists.length > 0)
            buyerFields = {"HenryJohnson": 7, "JesseBatey": 8, "KMaryland": 12, "KLouisiana": 13, "Other": 9};
        else
            buyerFields = {"HenryJohnson": 7, "JesseBatey": 8, "KMaryland": 12, "Other": 9};
        ...
        var result8;
        if (louisianaExists.length > 0)
            result8 = buildConnectionSet(newData, 'buyer_name', buyerFields, 'buyer_name', {"JesseBatey": 11, "HenryJohnson": 11, "KMaryland": 11, "KLouisiana": 11, "Other": 11}, [{name: 'ran_away', value: "1"}]);
        else
            result8 = buildConnectionSet(newData, 'buyer_name', buyerFields, 'buyer_name', {"JesseBatey": 11, "HenryJohnson": 11, "KMaryland": 11, "Other": 11}, [{name: 'ran_away', value: "1"}]);
        ...
        var datajson = {nodes: [
            ...
            {id: 12, name: "Maryland", color: "#00cdff"},
        ],
            links: all_results};
        if (louisianaExists.length > 0)
            datajson.nodes.push({id: 13, name: "Louisiana", color: "#d50053"});
